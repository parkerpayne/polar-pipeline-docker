...
#             opened.write('\t'.join(column_names) + '\n')
#             for row in rows:
#                 opened.write('\t'.join(map(str, row)) + '\n')
...
#     files = request.json.get("files")
#     parameters = request.json.get("params")
#     print(files, parameters, sep='\n')
#     maxprogress = len(files) * len(parameters)
#     print(maxprogress)
#     db_config_parser.read('/home/threadripper/shared_storage/databases/databases.ini')
...
#             os.remove(file)
#     columnType = {}
#     for line in open('/home/threadripper/shared_storage/databases/datatypes.ini'):
#         if line.startswith('['): continue
#         name, _type = line.strip().split('=')
#         match _type:
#             case 'str':
#                 columnType[name] = str
#             case 'int32':
...
#         file = db_config_parser['DEFAULT'][filename]
#         header = findHeaderLine(file)
#         # FIND DATA TYPES
#         columnIndex = {}
#         with open(searchname, 'a') as opened:
#             #DO COMPARISONS
#             opened.write('\t'.join(bob)+'\n')
#             foundHeader = False
#             for row in open(file, 'r'):
#                 if row == header:
#                     foundHeader = True
#                     for index, item in enumerate(row.strip().split('\t')):
...
#                             if str(bar) not in str(value):
#                                 valid = False
#                     progress = (fileIndex+1 * paramIndex+1) / maxprogress
#                 if valid:
#                     bob = []
#                     for col in columnType:
#                         if col in ['STRAND_1', 'TSL_1', 'APPRIS_1']:
#                             col = col.replace('_1', '')
#                         bob.append(line[columnIndex[col]])
#                     bob.append(filename)
#                     opened.write('\t'.join(bob)+'\n')
#     color = 'green'
#     return 'success'
...
#     color = 'spin'
#     searchname = f'{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}_search_result.tsv'
#     parameterList = request.json.get("params")
#     files = request.json.get("files")
...
#     print(parameterList)
#     print(files)
#     query = 'SELECT * FROM bedtable WHERE ("SOURCE" = \'' + '\' OR "SOURCE" = \''.join(files) + '\') AND '
#     parameters = []
#     for param in parameterList:
#         colname = param[2]
#         if param[0].startswith('1'):
#                     colname = '_'+param[0]
#         else:
#             colname = param[0]
#         colname = colname.replace('#', '').replace(')', '').replace('(', '_')
#         if not param[3]:
#             match param[2]:
#                 case '==':
#                     parameters.append(f'"{colname}" = \'{param[1]}\'')
#                 case 'Contains':
#                     parameters.append(f'"{colname}" LIKE \'%{param[1]}%\'')
#                 case '!=':
#                     parameters.append(f'NOT "{colname}" = \'{param[1]}\'')
#                 case default:
#                     parameters.append(f'"{colname}" {param[2]} \'{param[1]}\'')
#         else:
#             match param[2]:
#                 case '==':
#                     parameters.append(f'("{colname}" = \'{param[1]}\' OR "{colname}" IS NULL)')
#                 case 'Contains':
#                     parameters.append(f'("{colname}" LIKE \'%{param[1]}%\' OR "{colname}" IS NULL)')
#                 case '!=':
#                     parameters.append(f'(NOT "{colname}" = \'{param[1]}\' OR "{colname}" IS NULL)')
#                 case default:
#                     parameters.append(f'("{colname}" {param[2]} \'{param[1]}\' OR "{colname}" IS NULL)')
